    ...
    protected function task( $reviews ) {
        ...
        foreach ( $reviews as $index => $review ) {
            ...
            if( $count_cols < 6 ) {
                unset( $reviews[$index] );
                $results['errors']++;
                $results['error_list'][] = sprintf( __( 'Line %1$d >> Error: incorrect file format. Only %2$d column(s) found. Please open the input file in a text editor (e.g., in Notepad on Windows) and verify that columns are correctly separated by commas.', IVOLE_TEXT_DOMAIN ), $line_number, $count_cols );
                continue;
            }
